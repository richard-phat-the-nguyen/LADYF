...
def tubercle(z,corde ):
    A=0.05*corde
    Lambda=0.25*corde
    bord_attaque_tubercle=A*np.sin(2*np.pi*z/Lambda)
...
def run(context):
    ...
    try:
        ...
        #tubercle
        corde = bord_attaque(z2) - bord_fuite(z2)
        oscillation=tubercle(z2,corde)
        window_size = 5
        kernel = np.ones(window_size) / window_size
        oscillation = np.convolve(oscillation, kernel, mode='same')
        L_edge=bord_attaque(z2)+oscillation
        T_edge=bord_fuite(z2)
        ...
        for i in range(z2_npts):
            ...
            x_offset = bord_attaque(z_val)+oscillation[i]
